...
class LocationWorkspaceTests(TestCase):
    ...
    def test_workspace_list_ui_includes_bulk_and_row_helpers(self) -> None:
        ...
        self.assertContains(response, "data-copy-failed-label")
        self.assertContains(response, 'id="eco-row-%s"' % service.pk)
        self.assertContains(response, 'form="eco-row-%s"' % service.pk)
        self.assertContains(response, "eco-action-group-danger")
        self.assertContains(response, 'id="eco-copy-status"')
        self.assertContains(response, "eco-table-wrap")
        self.assertContains(response, "1 active")
        # One shared row form (not seven action-specific forms).
        self.assertNotContains(response, 'id="eco-row-%s-delete"' % service.pk)
        self.assertContains(response, 'aria-label="Delete Academy"')
        self.assertContains(response, 'aria-label="Move up"')
